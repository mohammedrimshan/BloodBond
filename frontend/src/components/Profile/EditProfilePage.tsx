...
const SectionHeader = ({ label }: { label: string }) => (
  <div className="flex items-center gap-3">
    <span
      className="text-[10px] uppercase tracking-[0.22em] font-bold text-[#9b4a35]"
    >
      {label}
    </span>
...
const Field = ({ label, icon, error, optional, children, className = "" }: FieldProps) => (
  <div className={`space-y-1.5 ${className}`}>
    <label
      className="flex items-center gap-1.5 text-[10px] uppercase tracking-widest font-bold text-[#9e7a6e]"
    >
      <span className="text-[#b07060]">{icon}</span>
      {label}
      {optional && (
        <span
          className="ml-1 normal-case tracking-normal text-[9px] px-1.5 py-0.5 rounded-full bg-[#f5ece8] text-[#c09080]"
        >
          optional
        </span>
      )}
...
const StyledInput = ({ hasError, className = "", ...props }: StyledInputProps) => (
  <input
    {...props}
